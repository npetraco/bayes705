...
void MyModel::from_prior(RNG& rng)
{
	...
	beta0 = 0 + 1*tan(M_PI*(_beta0 - 0.5));  // For Cauchy
	beta1 = 0 + 5*tan(M_PI*(_beta1/2.0));    // For Half-Cauchy
...
double MyModel::perturb(RNG& rng){
    ...
    beta0   = 0.0 + 1.0*tan(M_PI*(_beta0 - 0.5));
    beta1   = 0.0 + 5.0*tan(M_PI*(_beta1/2.0));
    epsilon = 0.0 + (1.0)*quantile(__boost_dist, _epsilon);
